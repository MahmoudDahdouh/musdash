...
23. `.pill` is `text-transform: lowercase` while every other label is sentence
    case.
24. Flash rendering is split: the layout has a flash block, but no route ever
    passed it a flash, while Settings renders its own from the view model. One
    path is enough. (Resolved in slice 6: Settings passes its flash to the
    layout, the first route to do so.)
...
- Headings: `h1` 24/1.2 600 tight; `h2` 16/1.3 600; `h3` 14/1.4 600. **No
  transform, no colour override.** (An `.eyebrow` uppercase label existed
  until slice 5b replaced its last use with a `.kv`; it was removed.) One
  exception: the `h1` inside the login and setup card is 20px (`--t-xl`), the
  wordmark's size, because 24px beside a 20px mark read as two titles.
...
- `@view-transition { navigation: auto }` inside the no-preference media query,
  with the root cross-fade at `--d-page` (200ms). The `a.card` hover transition
  lives in the same query; it was the one transition outside it until slice 7.
...
| Logs        | `.logs`                                                                | Height `min(70vh, 720px)` on the deployment page, `min(60vh, 560px)` on the tab. `scrollbar-gutter: stable`. Stays dark in both modes (a terminal is dark), with the colours moved to tokens `--term-bg` / `--term-fg` / `--term-err`. `.logs-empty` message rendered server-side as the `<pre>`'s first child when there are no lines, and hidden by CSS (`:not(:only-child)`) once the first line is appended — no script. "Jump to latest" becomes a `.btn.btn-sm` floating inside the panel's bottom-right rather than in the header, so it is next to where the eye is.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- **Log empty message**: none needed — CSS hides it (see Logs).
...
As built (slice 7): the column is `.narrow.auth` — `.narrow` itself is
unchanged because `main`'s error page uses it. The card holds the `h1` ("Sign
in"; on setup "Create the administrator account" and the single-user
sentence), then the error notice, then the form. The setup tagline reads
"Your $5 VPS runs your apps, not your dashboard." The fields and buttons are
unchanged from slice 2b.
...
- Domains: the add form as a labelled field ("Custom domain", with a hint)
  plus a default button on one row (landed in slice 5b). Still to do, carried
  past slice 5: the automatic domain as a `.kv` row with a "Copy" quiet
  button, and the custom list with a quiet danger "Remove".
...
- `h1` "Deployment" with the image reference in mono beneath it at `--t-sm`
  (finding 7), `.status` beside the heading, then a `.kv-strip` (a `dl` of
  `div`s): Started (UTC, to the minute, exact value in `<time datetime>`),
  Duration, Trigger, and Commit when there is one. "Back to resource" as a quiet button in the actions slot.
...
- A **status strip** at the top: three `.kv` rows with a `.status` each,
  Dashboard address (Set / Not set), HTTPS (first match of: Off with no host;
  Applying; Not checked; Unreachable; Not checked for a skipped DNS check;
  DNS points elsewhere; Waiting for DNS; On), GitHub (Connected / Not
  connected). The strip writes its labels itself, borrowing the
  `.status-*` tones; the status partial stays for resource states. Every value the page already computes. This is what the
...
- Notices re-toned: DNS mismatch and unreachable proxy stay `-error`; "no
  domain yet" and "applying" become `-info`; "env file is shadowed" and
  "does not resolve yet" become `-warn`.
- Restart: disabled per Button spec, with the reason as the hint beneath
  (the hint replaced the old notice; there is no restart notice).
- Known limits of the strip (slice 6): HTTPS "On" means applied, DNS pointing
  here and the proxy reaching the dashboard — musdash never sees the
  certificate. "Applying" shows only for a host saved on this page, because
  the bootstrap applies an env-set host without recording it; a permanently
  failed apply still pulses "Applying", and clearing the host reads "Off" at
  once even if the proxy has not been told yet. A DNS mismatch is an amber
  dot but a red notice, and behind a proxying CDN or NAT it never clears. The
  Restarting card reloads its own `?restarting=1` URL, so it reloads every
  six seconds until the user leaves the page (pre-existing). Moving the
  certificate-authority paragraph under the form changed "below" to "above".
...
| 5   | **Resource and deployment** | `resource.eta`, `deployment.eta`, `src/routes/app.ts` (pass `project`/`environment` to the deployment page; `navTree()` aggregation if the Core-Builder has landed it)                                                                  | Per §7. Overview `.kv` and clickable deployment rows. Logs empty message on both panels. Deployment breadcrumb is five levels. Error notice above the log. Handed over from slice 4: move the Variables editor to `.field` markup (as project.eta), delete the `.scope-boxes > label` rule, and relabel its Save button "Save variables" to match the confirm. **Result (5b):** Overview is a Details `dl.kv` (URL, Image, Previous image, Port, Health path, Memory); `.cols` goes two-up only when each column gets 360px, which also fixed the tablet overflow; both log panels have their heights, an empty message and a floating "Jump to latest"; the deployment page has five crumbs, an `h1` "Deployment", a `.kv-strip` and the error above the log (the route now passes project, environment and duration, and 404s without them); the resource Variables editor is on `.field` markup with "Save variables"; the domain input has a label and hint; every card title on these pages is `h2`; the tab bar fades at its end below 760px; the unused `pill` fields and `.eyebrow` are gone. Known: the deployment log shows server-rendered lines again when SSE replays them (pre-existing), and the page's Started, Duration and empty-log text are first-paint only — only the status updates live. app.css 30,422 B, app.js unchanged 14,803 B. | Log timestamps, copy button, search                                                                     |
| 6   | **Settings**                | `settings.eta`, `app.css`, `src/routes/app.ts` (GET `/settings` only), this plan                                                                                                                                                        | Status strip at top. Notices toned per §7. Prose under `<details>`, cards ≤ `--prose-w`. Restart disabled with reason as hint. **Result:** strip, re-toned notices, `<details>`, `.card-prose` cards; the flash moved into the layout (one hunk in the GET handler). app.css 30,569 B, app.js unchanged.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | New settings                                                                                            |
| 7   | **Auth pages and polish**   | `login.eta`, `setup.eta`, `app.css`, `layout.eta`                                                                                                                                                                                       | Login and setup per §7. `@view-transition` active and disabled under reduced motion. Optional: theme toggle in the sidebar foot (three-state, localStorage, `data-theme`, set before first paint by a 3-line inline script in `<head>`), recorded in `docs/DECISIONS.md` since PHASES.md §11 said "no toggle in Phase 1". **Result:** auth pages per §7; 200ms cross-fade, off under reduced motion; dead `h4` removed. Toggle not built, `layout.eta` untouched. app.css 31,295 B, app.js unchanged.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Anything that adds a request                                                                            |
...
**Final status.** Slices 0–7 are built, one commit each (slice 2 as 2a and
2b, slice 5 as 5a and 5b). app.css ends at 31,295 of 32,768 bytes, app.js at
14,803 of 16,384. The theme toggle was not built. Idle RSS after slice 7:
`PASS  idle RSS 39.8MB (ceiling 100MB).` — measured on macOS with Docker
unreachable and an empty database, so indicative only. The Linux `gate:rss`
run is still owed. Open, carried past their slices:

- the Domains tab's automatic domain as a `.kv` row with a Copy button, and a
  quiet "Remove";
- an invalid env line is echoed in `envError` (Core-Builder);
- the deployment log shows server-rendered lines again when SSE replays them;
- the Restarting card reloads every six seconds;
- `SettingsView.flash` is a pass-through that can go;
- `main`'s 403 `reachError` contradicts the firewall notice text;
- POST `/login` and `/setup` carry no CSRF token;
- restoring the pending state after back navigation is unverified in Firefox.
