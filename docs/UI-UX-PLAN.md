...
| 3   | **Shell**                   | `layout.eta`, `app.css`, `src/routes/app.ts` (`layout()` helper only)                                                                                                  | Wordmark and favicon per 6.4. Sidebar foot shows the RSS instrument labelled "dashboard", value from `process.memoryUsage.rss()` passed by `layout()`. Active nav item uses the ink bar (landed early, in slice 1). Page-head pattern with container-query wrapping in place on every page. Mobile drawer and bar restyled with the tokens.                                                          | Env status dots (needs `navTree()` to carry aggregated state; a Core-Builder change listed for slice 5) |
