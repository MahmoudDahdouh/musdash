...
Sentence case everywhere. Buttons name the outcome ("Save variables", not
...
:root {
  ...
  --r-lg: 14px;
  --r-full: 999px;
  --shadow-1: light-dark(0 1px 2px oklch(0% 0 0 / 0.06), none);
  --shadow-3: light-dark(0 12px 32px -8px oklch(0% 0 0 / 0.25), none);
...
`color-mix(in oklch, var(--ok) 14%, var(--surface))`. That keeps every tone
correct in both modes from one source value.
...
| ----------- | ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Button      | `.btn` + `.btn-primary` `.btn-danger` `.btn-quiet` `.btn-sm`           | 32px tall (28 for `-sm`), `--r-sm`, 500 weight. Default: `--surface`, `--border`, hover `--border-2` + `--surface-2`. Primary: `--ink`/`--on-ink`, hover lifts lightness 6%. Danger: filled `--danger`, white text, kept filled so the destructive choice never looks weaker than Cancel. Quiet: no border, hover `--surface-2`. `:disabled` and `[aria-busy]`: opacity 0.5, `cursor: not-allowed`, hover suppressed. `[aria-busy]` shows a 12px CSS spinner in the icon slot. `<a class="btn">` is identical and has no underline. |
| Icon button | `.btn.btn-icon`                                                        | Square 32px, `aria-label` required. Replaces `.iconbtn`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Field       | `.field` `.label` `.hint` `.error` `.opt`                              | `<div class="field"><label class="label" for=x>Name <span class="opt">optional</span></label><input id=x aria-describedby="x-hint"><p class="hint" id="x-hint">…</p></div>`. Inputs 36px tall, `--surface` background (not `--bg`), `--border`, focus per 6.2. `:user-invalid` swaps the border to `--danger` and reveals `.error` if present, so the browser's own validation gets inline styling with no JS. Textareas mono. `.checkline` stays for checkbox rows.                                                                |
| Card        | `.card` + `.card-title` `.card-body` `.card-foot`                      | `--surface`, `--border`, `--r-md`, `--shadow-1`, padding `--s-5`. `a.card` hover: `--border-2` and `--shadow-1` deepened; focus ring per 6.2; no transform. `.card-danger` tints the border from `--danger` at 40%.                                                                                                                                                                                                                                                                                                                 |
| Status      | `.status` + `.status-<state>`                                          | Inline-flex, 8px dot (`::before`), sentence-case label from the template. Dot colour by state: healthy/succeeded `--ok`, unhealthy `--warn`, failed `--danger`, deploying/running `--busy`, queued/stopped `--idle` hollow. `.status-deploying` and `.status-queued` pulse. The Alpine bindings change from `'pill-' + state` to `'status-' + state`; the label text comes from a small map in the template so the JSON state stays as it is.                                                                                       |
| Chip        | `.chip` + `.chip-<tone>`                                               | Tinted background via `color-mix`, dark text of the same hue, 12px, `--r-full`. Used for env scope (`runtime`/`build`/`both`), the `private` repo marker, and origin (`.chip-outline`).                                                                                                                                                                                                                                                                                                                                             |
| Notice      | `.notice` + `.notice-info` `.notice-ok` `.notice-warn` `.notice-error` | Replaces `.flash`. Left icon from the sprite (`i-info`, `i-check`, `i-alert`, `i-x-circle`, four new symbols), tinted background, border from the tone at 40%, text `--text` (not the tone colour, which fails contrast on a tint). `role="status"` for info/ok, `role="alert"` for warn/error. Rendered by the layout only; pages pass a notice through the layout data rather than rendering their own.                                                                                                                           |
| Table       | `.table` inside `.table-scroll`                                        | One rule set. 13px, header `--text-2` 500 with a bottom `--border`, rows 40px with `--border` dividers, hover `--surface-2`, `tabular-nums`. `.table-scroll` is always the wrapper. A row whose first cell is a link gets `cursor: pointer` on the row and the whole row navigates via a 4-line delegated click handler that respects modifier keys.                                                                                                                                                                                |
...
| Empty state | `.empty`                                                               | Icon from the sprite, `h2`, one paragraph at `--prose-w`, one primary action. Compact variant `.empty-sm` for inside a section.                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Sidebar     | `.sidebar` `.nav-*`                                                    | Unchanged structure. Brand row gets the wordmark (6.4). Env rows get a 6px status dot when the nav data carries state (see 8, slice 3). Foot gains the RSS instrument. Active item: `--surface-2` background and `--text`, a 2px `--ink` bar on the left edge instead of a blue tint.                                                                                                                                                                                                                                               |
| Page head   | `.page-head`                                                           | Breadcrumb row (quiet links, `/` separators from `::before`), then a title row: `h1` + status on the left, an `.actions` group on the right that wraps below 600px of container width. Uses a container query, not a viewport query, because the sidebar makes viewport width the wrong measure.                                                                                                                                                                                                                                    |
...
- **Pending state** (~12 lines): on any form `submit` that is not prevented,
  ...
  button.
- **Dialog openers** (~6 lines): `data-open="dialog-id"` delegated click
  handler replaces every inline `onclick`. Buttons also carry
  `commandfor`/`command="show-modal"` so browsers with Invoker Commands need
  no JS at all; the handler skips when `"command" in HTMLButtonElement.prototype`.
- **Row links** (~4 lines): see Table.
...
- Resource card: name (UI face, sentence case, finding 8) and `.status` on
...
| 3   | **Shell**                   | `layout.eta`, `app.css`, `src/routes/app.ts` (`layout()` helper only)                                                                                                  | Wordmark and favicon per 6.4. Sidebar foot shows the RSS instrument labelled "dashboard", value from `process.memoryUsage.rss()` passed by `layout()`. Active nav item uses the ink bar. Page-head pattern with container-query wrapping in place on every page. Mobile drawer and bar restyled with the tokens.                                                                                     | Env status dots (needs `navTree()` to carry aggregated state; a Core-Builder change listed for slice 5) |
| 4   | **Projects and project**    | `projects.eta`, `project.eta`                                                                                                                                          | Per §7. Resource names render in the case they were typed. Variables tab keys appear as chips. Every dialog uses `data-open`/`commandfor`.                                                                                                                                                                                                                                                           | Resource page                                                                                           |
| 5   | **Resource and deployment** | `resource.eta`, `deployment.eta`, `src/routes/app.ts` (pass `project`/`environment` to the deployment page; `navTree()` aggregation if the Core-Builder has landed it) | Per §7. Overview `.kv` and clickable deployment rows. Logs empty message on both panels. Deployment breadcrumb is five levels. Error notice above the log.                                                                                                                                                                                                                                           | Log timestamps, copy button, search                                                                     |
...
| Need                                                     | Who          | Notes                                                                                                                                   |
| -------------------------------------------------------- | ------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
| `rss` in layout data                                     | UI-Builder   | `process.memoryUsage.rss()` in `layout()`; in scope for `src/routes/**`.                                                                |
| `project`, `environment` on the deployment page          | UI-Builder   | Already computed as `ctx` in the handler.                                                                                               |
| Aggregated resource state per environment in `navTree()` | Core-Builder | One extra column in the existing nav query (worst state among the env's resources). Enables the sidebar dots. Not required for slice 3. |
