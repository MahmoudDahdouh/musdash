...
| Table       | `.table` inside `.table-scroll`                                        | One rule set. 13px, header `--text-2` 500 with a bottom `--border`, rows 40px with `--border` dividers, hover `--surface-2` on rows that link somewhere, `tabular-nums`. `.table-scroll` is always the wrapper. A row whose first cell is a link gets `cursor: pointer` on the row and the whole row navigates via a 4-line delegated click handler that respects modifier keys.                                                                                                                                                                                                                                          |
